(function($) {
	...
	elFinder.prototype.options = {
		...
		toolbar        : [
			...
			['copy', 'paste', 'rm'],
			['rename', 'edit', 'editsrc'],
			['info', 'quicklook', 'download', 'resize'],
			['icons', 'list'],
			['help']
		],
		...
		contextmenu : {
			...
			'file'  : ['select', 'open', 'quicklook', 'delim', 'copy', 'cut', 'rm', 'delim', 'duplicate', 'rename', 'edit', 'editsrc', 'resize', 'archive', 'extract', 'delim', 'download', 'info'],
	...
	*/
	this.renderCwd = function() {
		...
		for (var hash in this.fm.cdc) {
			if(this.fm.cdc[hash].show==false) continue;
			num++;
...
elFinder.prototype.ui = function(fm) {
	...
	this.init = function(disabled) {
		...
		if(this.fm.options.docked) {
			if(this.fm.options.allowDocking) {
...
elFinder.prototype.ui.prototype.commands = {
	...
	open : function(fm) {
		...
		this.exec = function(dir) {
			...
			function openFile(f) {
				...
				if(fm.options.onOpen=="window") {
					window.open(self.fm.options.url+'&cmd=open&current='+(f.parent||self.fm.cwd.hash)+'&target='+(f.link||f.hash), false, 'top=50,left=50,'+ws+'scrollbars=yes,resizable=yes');
				} else if(typeof fm.options.onOpen == "function") {
					fm.options.onOpen(self.fm.options.url+'&cmd=open&current='+(f.parent||self.fm.cwd.hash)+'&target='+(f.link||f.hash),f.name);
				} else if(fm.options.onOpen.length>0) {
					window[fm.options.onOpen](self.fm.options.url+'&cmd=open&current='+(f.parent||self.fm.cwd.hash)+'&target='+(f.link||f.hash));
				} else {
					window.open(self.fm.options.url+'&cmd=open&current='+(f.parent||self.fm.cwd.hash)+'&target='+(f.link||f.hash), false, 'top=50,left=50,'+ws+'scrollbars=yes,resizable=yes');
				}
			}
	...
	download : function(fm) {
		...
		this.exec = function() {
			var f = this.fm.getSelected(0);
			//console.log(this.fm.options);
			//this.fm.cwd.rel+"/"+f.name
			var s, ws = '';
			if (f.dim) {
				s  = f.dim.split('x');
				ws = 'width='+(parseInt(s[0])+20)+',height='+(parseInt(s[1])+20)+',';
			}
			window.open(self.fm.options.url+'&cmd=download&current='+(f.parent||self.fm.cwd.hash)+'&target='+(f.link||f.hash));
		};
	...
	editsrc : function(fm) {
		...
		this.exec = function() {
			...
			if(fm.options.onEditSrc=="window") {
				window.open(self.fm.options.url+'&cmd=editsrc&current='+(f.parent||self.fm.cwd.hash)+'&target='+(f.link||f.hash), false, 'top=50,left=50,'+ws+'scrollbars=yes,resizable=yes');
			} else if(typeof fm.options.onEditSrc == "function") {
				fm.options.onEditSrc(self.fm.options.url+'&cmd=editsrc&current='+(f.parent||self.fm.cwd.hash)+'&target='+(f.link||f.hash), f.name);
			} else if(fm.options.onEditSrc.length>0) {
				window[fm.options.onEditSrc](self.fm.options.url+'&cmd=editsrc&current='+(f.parent||self.fm.cwd.hash)+'&target='+(f.link||f.hash), f.name);
			} else {
				$('<div />').append("Source Editor Not Found").dialog({
						dialogClass : 'el-finder-dialog el-finder-dialog-info',
						width       : 390,
						position    : ['50%','50%'],
						title       : self.fm.i18n(f.mime == 'directory' ? 'Folder info' : 'File info'),
	...
	edit : function(fm) {
		...
		this.exec = function() {
			...
			this.fm.ajax({
				...
				self.fm.lockShortcuts(true);
				var ta = $('<textarea id=fsCodeEditor/>').val(data.content||'').keydown(function(e) {
					e.stopPropagation();
					if (e.keyCode == 9) {
						e.preventDefault();
						if ($.browser.msie) {
				...
				$('<div style="padding:4px;overflow:hidden;" />').append(ta)
					.dialog({
						dialogClass : 'el-finder-dialog',
