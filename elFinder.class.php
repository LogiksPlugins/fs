...
class elFinder {
	...
	protected function _open()
	{
		if (isset($_GET['current'])) { // read file
			...
			//ToDo :: Tweak For Opening All Files That Are Text Type
			if(strpos($mime,"text")==0) $mime="text";
	...
	protected function _edit()
	{
		...
		$data=$_POST['content'];
		$data=cleanText($data);
		if (false === file_put_contents($target, trim($data))) {
	...
	protected function _info($path)
	{
		...
		$show=true;
		if(!$this->_options['showNoAccess']) {
			$show=!(!$this->_isAllowed($path, 'read') && !$this->_isAllowed($path, 'write') && !$this->_isAllowed($path, 'rm'));
		}
		$info = array(
	 ...
	 * @return string
	 **/
	public function _findDir($hash, $path='')
	{
		if (!$path) {
			$path = $this->_options['root'];
